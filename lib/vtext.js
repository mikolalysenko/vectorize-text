...
function parseTag(tag, TAG_CHR, str, map) {
  ...
  while (a > -1) {
    ...
    if(isRecursive) {
      var start = a + nOPN
      var len = b + nCLS - start

      var new_str = str.substr(start, len)
      var new_map = map.slice(start, start + len)

      console.log("new_str=", new_str);
      console.log("new_map=", new_map);

      var newParse = parseTag(tag, TAG_CHR, new_str, new_map).map
      new_str = newParse.str
      new_map = newParse.map

      for(var k = 0; k < new_map.length; ++k) {
        map[k + a + nOPN] = new_map[k]
      }
      str = str.substr(0, start) + new_str + str.substr(start + len)
    }
...
function getPixels(canvas, context, raw_str, fontSize, lineSpacing) {
  ...
  map = parseTag(TAG_bold,
                 CHR_bold, raw_str, map).map
  map = parseTag(TAG_italic,
                 CHR_italic, raw_str, map).map
  map = parseTag(TAG_super,
                 CHR_super, raw_str, map).map
  map = parseTag(TAG_sub,
                 CHR_sub, raw_str, map).map
  ...
  for(i = 0; i < numberOfLines; ++i) {
    ...
    function changeStyle(oldStyle, newStyle) {
      ...
      var wasSub = (oldStyle.indexOf(CHR_sub0) > -1)
      var is_Sub = (newStyle.indexOf(CHR_sub0) > -1)
      ...
      var wasSuper = (oldStyle.indexOf(CHR_super0) > -1)
      var is_Super = (newStyle.indexOf(CHR_super0) > -1)
