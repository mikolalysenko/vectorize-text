...
function getPixels(canvas, context, raw_str, fontSize, lineSpacing) {
  ...
  for(j = 0; j < raw_str.length; ++j) {
    if(map[j] !== null) {
      plainText += raw_str[j]
      styleOptions.push(map[j])
    }
  }

console.log("raw_str=", raw_str);
console.log("map=", map);
console.log("allText=", plainText);
console.log("styleOptions=", styleOptions);
console.log("raw_str.length=", raw_str.length);
console.log("map.length=", map.length);
console.log("allText.length=", plainText.length);
console.log("styleOptions.length=", styleOptions.length);

  var allText = plainText.split('\n')

  var numberOfLines = allText.length
  ...
  for(i = 0; i < numberOfLines; ++i) {
    ...
    for(j = 0; j < txt.length; ++j) {
      var restyle = false
      var newStyle
      if(activeStyle === styleOptions[j + nDone]) {
        buffer += txt[j]

        if(j === txt.length - 1) {
          if(j + nDone + 2 < styleOptions.length) {
            if(activeStyle !== styleOptions[j + nDone + 2]) {
              restyle = true
              newStyle = styleOptions[j + nDone + 2]
            }
          }
        }
      } else {
        ...
        buffer = txt[j]
        restyle = true
        newStyle = styleOptions[j + nDone]
      }

      if(restyle) {
        changeStyle(activeStyle, newStyle)
        activeStyle = newStyle
      }
    }
    writeBuffer()

    nDone += txt.length + 1 // added one for enter
