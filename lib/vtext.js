...
function getPixels(canvas, context, raw_str, size) {

  var str

  var allStr = raw_str.split('\n')

  var minHeight = size * 3

  if(canvas.height < minHeight) {
    canvas.height = minHeight
  }

  //var lineHeight = Math.ceil(1.5 * size)
  //var lineHeight = Math.ceil(2.0 * size)
  var lineHeight = Math.ceil(2.25 * size)
  //var lineHeight = Math.ceil(2.5 * size)
  //var lineHeight = Math.ceil(3.0 * size)
  ...
  var allPixels = new Uint8ClampedArray(0);
  ...
  for(var i = 0; i < allStr.length; ++i) {
    ...
    context.fillText(str, offsetX, offsetY)

    var oldPixels = allPixels;

    //Cut pixels from image
    var yCut = lineHeight
    //var yCut = canvas.height

    var newPixels = context.getImageData(0, 0, maxWidth, yCut).data

    allPixels = new Uint8ClampedArray(oldPixels.length + newPixels.length)
    allPixels.set(oldPixels)
    allPixels.set(newPixels, oldPixels.length)
  }

  var pixels = ndarray(allPixels, [yCut * allStr.length, maxWidth, 4])
