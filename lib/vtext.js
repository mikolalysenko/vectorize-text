...
function parseTag(tag, TAG_CHR, str, map) {
  ...
  var b = -nCLS
  while (a > -1) {
    //if (b + nCLS >= nStr) break
    a = str.indexOf(opnTag, b + nCLS)
    if(a === -1) break

    //if (a + nOPN >= nStr) break
...
function getPixels(canvas, context, raw_str, fontSize, lineSpacing) {
  ...
  var lineHeight = Math.ceil(lineSpacing * fontSize)
  ...
  for(i = 0; i < numberOfLines; ++i) {
    ...
    function changeStyle(oldStyle, newStyle) {
      ...
      var wasBold = (oldStyle.indexOf(CHR_italic) > -1)
      var is_Bold = (newStyle.indexOf(CHR_italic) > -1)
    ...
    for(j = 0; j < txt.length; ++j) {
      if((j < 1) || (styleOptions[j] === activeStyle)) {
        buffer += txt[j]
      } else {
        writeBuffer()
        buffer = txt[j]
        changeStyle(activeStyle, styleOptions[j])
        activeStyle = styleOptions[j]
      }
    }
    writeBuffer()

    var width = Math.ceil(xPos + 2 * offsetX) | 0
